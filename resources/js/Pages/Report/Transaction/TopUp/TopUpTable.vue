...
const first = ref(0);
const totalTopUpAmount = ref();
const topUpCounts = ref();
...
const loadLazyData = (event) => { // event will retrieve from the datatable attribute
    ...
    try {
        setTimeout(async () => {
            ...
            const url = route('report.transaction.get_top_up_listing_data', params);
            const response = await fetch(url);

            const results = await response.json();
            topUps.value = results?.data?.data;
            totalRecords.value = results?.data?.total;
            totalTopUpAmount.value = results?.totalTopUpAmount;
            topUpCounts.value = results?.topUpCounts;
...
const emit = defineEmits(['updateTopUp']);

// Emit the totals whenever they change
watch([totalTopUpAmount, topUpCounts], () => {
    emit('updateTopUp', {
        totalTopUpAmount: totalTopUpAmount.value,
        topUpCounts: topUpCounts.value,
    });
});
...
//filter status
const status = ref(['success', 'failed', 'rejected']);

// filter transaction type
const type = ref(['top_up', 'top_up_capital'])
                ...
                <template v-if="topUps?.length > 0">
                    ...
                    >
                        <template #body="{ data }">
                            <Tag
                                severity="secondary"
                                :value="$t(`public.${data.transaction_type}`)"
                            />
                        </template>
                    </Column>
