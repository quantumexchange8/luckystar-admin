...
const first = ref(0);
const totalRedemptionAmount = ref();
const redemptionCounts = ref();
...
const loadLazyData = (event) => { // event will retrieve from the datatable attribute
    ...
    try {
        setTimeout(async () => {
            ...
            const url = route('report.transaction.get_redemption_listing_data', params);
            const response = await fetch(url);

            const results = await response.json();
            redemptions.value = results?.data?.data;
            totalRecords.value = results?.data?.total;
            totalRedemptionAmount.value = results?.totalRedemptionAmount;
            redemptionCounts.value = results?.redemptionCounts;
...
const emit = defineEmits(['updateRedemption']);

// Emit the totals whenever they change
watch([totalRedemptionAmount, redemptionCounts], () => {
    emit('updateRedemption', {
        totalRedemptionAmount: totalRedemptionAmount.value,
        redemptionCounts: redemptionCounts.value,
    });
});
