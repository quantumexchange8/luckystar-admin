...
const loadLazyData = (event) => { // event will retrieve from the datatable attribute
    ...
    try {
        setTimeout(async () => {
            ...
            const url = route('report.transaction.get_transfer_listing_data', params);
            const response = await fetch(url);

            const results = await response.json();
            transfers.value = results?.data?.data;
            totalRecords.value = results?.data?.total;
            totalTransferAmount.value = results?.totalTransferAmount;
            transferCounts.value = results?.transferCounts;
...
const emit = defineEmits(['updateTransfer']);

// Emit the totals whenever they change
watch([totalTransferAmount, transferCounts], () => {
    emit('updateTransfer', {
        totalTransferAmount: totalTransferAmount.value,
        transferCounts: transferCounts.value,
    });
});
