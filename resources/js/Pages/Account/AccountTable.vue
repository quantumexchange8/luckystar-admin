...
import AccountTableAction from "@/Pages/Account/AccountTableAction.vue";
...
watch([filters.value['status'], filters.value['group_id'], filters.value['account_type']], () => {
    loadLazyData()
});
...
<!--                                class="flex gap-3 items-center w-full md:w-fit"-->
<!--                                @click="exportReport"-->
<!--                                :disabled="exportStatus || !accounts.length"-->
<!--                            >-->
...
<!--                            </Button>-->
                ...
                <template #loading>
                    <div class="flex flex-col gap-2 items-center justify-center">
                        <ProgressSpinner
                            strokeWidth="4"
                        />
                        <span class="text-sm text-surface-700 dark:text-surface-300">{{ $t('public.loading_accounts_caption') }}</span>
                    </div>
                </template>
                <template v-if="accounts?.length > 0">
                    <Column
                        ...
                        :header="$t('public.last_update')"
                    >
                        <template #body="{data}">
                            {{ dayjs(data.updated_at).format('YYYY-MM-DD') }}
                            <div class="text-xs text-surface-500">
                                {{ dayjs(data.updated_at).format('HH:mm:ss') }}
                            </div>
                        </template>
                    </Column>
                    <Column
                    ...
                    >
                        <template #body="{data}">
                            <div class="flex flex-nowrap gap-2 items-center">
                                <span class="font-medium">{{ data.meta_login }}</span>
                                <Tag
                                    v-if="data.trading_master"
                                    severity="info"
                                    :value="$t('public.strategy')"
                                    class="text-xxs"
                                />
                            </div>
                        </template>
